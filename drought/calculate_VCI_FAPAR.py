...
# Collect all August NDVI files from 2016 to 2024
august_files = []
for year in range(2016, 2025):
    year_dir = os.path.join(base_dir, str(year))
    if os.path.exists(year_dir):
        files = glob.glob(os.path.join(year_dir, "Lincs_*_Aug_NDVI.tif"))
        if files:
...
if len(august_files) < 2:
    print("Error: Insufficient August NDVI files found to compute VCI.")
    exit(1)

print(f"Found {len(august_files)} August NDVI files.")

# Read NDVI arrays for all August files
ndvi_arrays = []
for file in august_files:
    data, _ = read_ndvi_array(file)
    if data is not None:
        ndvi_arrays.append(data)

# Stack the arrays and compute NDVI_min and NDVI_max
...
if august_2022_file is None:
    print("Error: August 2022 NDVI file not found.")
    exit(1)

print(f"August 2022 file: {august_2022_file}")
ndvi_2022, dataset_2022 = read_ndvi_array(august_2022_file)
if ndvi_2022 is None:
    print("Error: Could not read August 2022 NDVI data.")
    exit(1)

# Compute VCI
denominator = ndvi_max - ndvi_min
vci = np.where((denominator > 0) & (~np.isnan(ndvi_2022)),
               (ndvi_2022 - ndvi_min) / denominator * 100,
               np.nan)
...
# Save plot
plot_file = 'Lincs_VCI_2022_Aug_plot.png'
