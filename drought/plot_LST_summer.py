...
monthly_data = {abbr: {"years": [], "fapar": []} for abbr in month_map.keys()}
    ...
    for file_path in files:
        ...
        if mean_fapar is not None:
            ...
            if month_abbr in monthly_data:
                ...
                monthly_data[month_abbr]["fapar"].append(mean_fapar)
...
for month_abbr in month_map.keys():
    ...
    fapar = monthly_data[month_abbr]["fapar"]
    ...
    # Sort by year
    sorted_pairs = sorted(zip(years, fapar))
    sorted_years, sorted_fapar = zip(*sorted_pairs)

    # Create the plot
    plt.figure(figsize=(12, 6))
    plt.plot(sorted_years, sorted_fapar, marker='o', linestyle='-', color='purple')
    plt.title(f"{month_abbr} Mean FAPAR Lincolnshire (2016–2024)", fontsize=18)
    ...
    # Save plot
    out_filename = f"./figures/fapar_{month_abbr}_time_series.png"
